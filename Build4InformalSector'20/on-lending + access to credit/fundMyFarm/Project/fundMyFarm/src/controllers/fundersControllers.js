...
// register
const funderApplication = async (req, res, next) => {
    passport.authenticate('registerFunder', (err, user, info) => {
      if (err) {
      ...
      if (info !== undefined) {
        ...
        req.logIn(user, error => {
          ...
          const { title, address, town, state, fullName, gender, email, phone,  country, image,  } = req.body
...
// sign-in
const funderLogin = async (req, res, next) => {
      passport.authenticate('loginFunder', (err, users, info) => {
        if (err) {
...
const findUser = async (req, res, next) => {
  passport.authenticate('jwtFunder', { session: false }, (err, user, info) => {
    if (err) {
      console.log(err);
    }
    if (info !== undefined) {
     ...
     Funder.findOne({
        ...
        if (userInfo != null) {
          ...
          const { title, address, town, state, fullName, gender, email, phone,  country, image, password, } = userInfo
          res.status(200).send({
            auth: true,
